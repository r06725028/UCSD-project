...
function isMaster(name) {
  ...
  if (outline) {
    ...
    towhite = "fill"
      ...
      node.append("text")
      .html(function (d) {
        dx = d.value < bigNode ? 0.2 + 0.5 * d.value.toString() + "em" : 0.0 
        rNames = d.name.split('@#$').slice(1)[0].split('\/')
        tspanList = rNames.map((name) => {
          return "<tspan x='0' dy='1.5em' dx=" + dx + ">" + name + "</tspan>" 
        })
      ...
      node.on("click", function(d) {
        if (d3.event.defaultPrevented === true) {
          ...
          gid=d.name.split('@#$')[0].replace(/SCR_0*/g, '')
          window.open(gid+'_main.html', "_blank"); // <== remove base URL eventually
        }
      })
