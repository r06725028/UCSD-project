...
#把空的confidence轉為NULL，所以此欄是TEXT
men_df = men_df.fillna({"CONFIDENCE": "NULL"}) 

"""
#men_df.to_csv("testnotnull.csv",sep='\t')
#檢查id是否有非數字出現（原本的mention.tsv中有部分資料格式錯誤，會導致not null failed）
#csvfile = open('test.csv', 'r')
#men_df = pd.read_csv(csvfile, delimiter='\t', low_memory=False)
for index, idnum in enumerate(men_df['ID']):
	for char in str(idnum):
		if char not in num_list:
			print("error index: ", str(index))
			print("idnum: ", str(idnum))
			break
print("id not string check ok")
for index, conf in enumerate(men_df['CONFIDENCE']):
	if conf not in conf_list:
		print("error index: ", str(index))
		print("id: ", men_df['ID'][index])
		print("conf: ", str(conf))
print("confidence not null check ok")
"""		
